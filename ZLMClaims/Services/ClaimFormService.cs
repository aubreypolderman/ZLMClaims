...
public class ClaimFormService : IClaimFormService
{
    ...
    public async Task DeleteClaimFormAsync(int id)
    {
        HttpResponseMessage response = await _httpClient.DeleteAsync($"{baseUrl}/api/ClaimForms/{id}");
        var responseContent = await response.Content.ReadAsStringAsync();
        Debug.WriteLine(DateTime.Now + "[.........][ClaimFormService] [DeleteClaimFormAsync] Response: " + responseContent);

        if (!response.IsSuccessStatusCode)
        {
            Debug.WriteLine(DateTime.Now + "[.........][ClaimFormService] [DeleteClaimFormAsync] Error deleting claimform with id {id}:  {response.ReasonPhrase}");
            throw new HttpRequestException($"Error deleting claim form with id {id}: {response.ReasonPhrase}");
        }
    }
}
